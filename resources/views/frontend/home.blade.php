  ...
  <div class="row">
        ...
        <ul class="nav flex-column nav-pills" id="categoryTab" role="tablist" aria-orientation="vertical">
          @foreach($categories as $key => $category)
            <li class="nav-item" role="presentation">
              <a class="nav-link {{ $key == 0 ? 'active' : '' }}" id="cat{{ $category->id }}-tab" data-bs-toggle="pill" href="#cat{{ $category->id }}" role="tab" aria-controls="cat{{ $category->id }}" aria-selected="false">{{ $category->name }}</a>
            </li>
          @endforeach
        </ul>
    ...
    <div class="col-md-9">
      <div class="tab-content" id="categoryTabContent">
        @foreach($categories as $key => $category)
        <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="cat{{ $category->id }}" role="tabpanel" aria-labelledby="cat{{ $category->id }}-tab">  
          ...
          <div class="card mb-3">
            <div class="card-body">
              ...
              <a href="{{ asset('uploads/posts/'.$post->file_attach) }}" target="_blank" class="download-link" title="Tải tài liệu"><i class="fa fa-download"></i></a>
            </div>
          </div>
          @endforeach
        </div>
